...
import es.ual.node.recovery.adapters.in.web.OriginManifestKeepListResponsePayload;
...
/**
 * Signed HTTP client (tutor-side) for {@code GET /ops/tutor/manifest-keep-list}. Tutor pings the
 * origin asking for the whitelist of file ids it currently has; unmentioned manifests in tutor
 * custody get purged.
 */
public class SignedHttpRemoteOriginKeepListClient implements RemoteOriginKeepListClientPort {

  private static final Logger LOGGER =
      LoggerFactory.getLogger(SignedHttpRemoteOriginKeepListClient.class);
  private static final String PATH = "/ops/tutor/manifest-keep-list";
  ...
  @Override
  public List<String> fetchKeepList(final String originBaseUrl) {
    ...
    final String nonce = "manifest-keep-list-" + UUID.randomUUID();
    ...
    try {
      ...
      if (response.statusCode() < 200 || response.statusCode() >= 300) {
        throw new IllegalStateException(
            "Manifest keep-list failed HTTP " + response.statusCode() + " body=" + response.body());
      }
      final OriginManifestKeepListResponsePayload payload =
          objectMapper.readValue(response.body(), OriginManifestKeepListResponsePayload.class);
      if (payload == null || payload.fileIds() == null) {
        return List.of();
      }
      return List.copyOf(payload.fileIds());
    } catch (IOException ex) {
      throw new IllegalStateException("Manifest keep-list I/O error from " + uri, ex);
    } catch (InterruptedException ex) {
      Thread.currentThread().interrupt();
      throw new IllegalStateException("Manifest keep-list interrupted from " + uri, ex);
    } finally {
      LOGGER.atDebug().setMessage("Manifest keep-list completed").addKeyValue("uri", uri).log();
    }
  }
